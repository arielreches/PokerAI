...
class Kunh:
    ...
    def cfr(self, history, pr_1, pr_2):
     if(terminal):
         return get_reward(history, history[-1], history[-2])

    strategy = self.strategy

    if_p1:
        util_1 = -1 * cfr(self, history+action1, pr1*strategy, pr2)
        util_2 = -1 * cfr(self, history+action2, pr1*strategy, pr2)
    if_p2
        util_1 = -1 * cfr(self, history + action1, pr1, pr2*strategy)
        util_2 = -1 * cfr(self, history + action2, pr1, pr2*stratrgy)

    utils = sum(strategy * [util1, util2])
    regrets = [(util_1 - util), (util_2 - util)]

    if_p1:
        cum_regrets += regrets * pr2
        pr += pr1
